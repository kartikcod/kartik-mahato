...
export default function Footer() {
  ...
  const socialLinks = [
    ...
    {
      name: "LinkedIn",
      url: "https://www.linkedin.com/in/kartik-react-developer/",
    },
  ...
  const handleCopy = async (e: React.MouseEvent) => {
    e.preventDefault(); // Default mailto behavior ko rokne ke liye
    try {
      await navigator.clipboard.writeText(email);
      setCopied(true);

      // 2 seconds baad status reset karne ke liye
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 md:px-12 pt-40 pb-12">
        ...
        >
          ...
          <div className="md:col-span-7 grid grid-cols-2 md:grid-cols-3 gap-8">
            <div>
              <p className="text-xs uppercase text-zinc-600 font-bold mb-6 tracking-widest">
                Navigation
              </p>
            ...
            <div className="col-span-2 md:col-span-1 relative">
              ...
              <div className="relative inline-block group">
                <motion.button
                  onClick={handleCopy}
                  whileHover={{ x: 5 }}
                  className="text-lg text-gray-400 hover:text-violet-400 transition-colors break-words text-left flex items-center gap-2"
                >
                  {email}
                  {/* Subtle icon indicator */}
                  <span className="absolute mb-12 text-[10px] opacity-0 group-hover:opacity-50 transition-opacity uppercase font-black">
                ...
                <AnimatePresence>
                  {copied && (
                    <motion.span
                      ...
                      className="absolute left-0 -top-8 bg-violet-600 text-white text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-tighter shadow-lg z-50"
                    >
                      Email Copied!
                    </motion.span>
                  )}
